...
async function searchUsers(appId, appSecret, name) {
  const token = await getAccessToken(appId, appSecret);
  
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'open.feishu.cn',
      path: '/open-apis/search/v4/users/simplify?query=' + encodeURIComponent(name),
      method: 'GET',
      headers: {
...
// Search for 邱世乐
searchUsers(appId, appSecret, '世乐')
  .then(result => console.log('Search result:', JSON.stringify(result, null, 2)))
